...
/*   Updated: 2021/05/28 13:28:15 by seonhjeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_resolution(char *line, t_window *window)
{
	...
	if (buf == NULL)
	{
		printf("Error\nFailed line-spliting at get_resolution\n");
		return (ERROR);
	}
...
int		get_texture_dir(char *line, t_window *window)
{
	...
	if (buf == NULL)
	{
		printf("Error\nFailed line-spliting at get_texture_dir\n");
		return (ERROR);
	}
...
int		get_color(char *line, t_window *window)
{
	...
	if (!buf || !code)
	{
		printf("Error\nFailed line-spliting at get_color\n");
